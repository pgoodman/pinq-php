...
abstract class DatabaseModelDefinition extends ModelDefinition {
	...
	/**
	 * Get a record from the database. This will deal with any sub-records of
	 * the record.
	 */
	public function getRecord(array &$data = array()) {
		...
		if(isset($data['__pql__'])) {
			...
			do {
				...
				if(NULL === ($model = $this->model_loader->load($model_name))) {
	...
	public function getModelGateway(DataSource $ds) {
		return new DatabaseModelGateway($ds, $this->model_loader, $this);
	}
}
